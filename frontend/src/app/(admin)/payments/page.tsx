...
import { 
  getPayments, Payment, PaymentFilters
} from '@/api/payments';
...
export default function PaymentsPage() {
  ...
  const fetchPayments = async () => {
    ...
    try {
      ...
      // Ensure we always have a valid array, even if the API response is unexpected
              ...
              {loading ? (
                ...
                payments && payments.map((payment) => (
                  <TableRow key={payment.id} hover>
                    ...
                    <TableCell>
                      {payment.createdAt && format(new Date(payment.createdAt), 'dd MMM yyyy', { locale: id })}
                    </TableCell>
                    <TableCell>Rp {payment.amount ? payment.amount.toLocaleString() : '0'}</TableCell>
                    <TableCell>{payment.method ? (methodLabels[payment.method] || payment.method) : '-'}</TableCell>
                    <TableCell>
                      <Chip 
                        label={payment.status ? (statusLabels[payment.status] || payment.status) : '-'} 
                        color={payment.status ? (statusColors[payment.status] as "default" | "primary" | "secondary" | "error" | "info" | "success" | "warning") || "default" : "default"}
                        size="small"
                      />
                    </TableCell>
